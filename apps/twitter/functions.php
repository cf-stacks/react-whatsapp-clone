...
function sb_twitter_subscribe() {
    $twitter = new TwitterAPIExchange(sb_get_setting('twitter'));
    $response = json_decode($twitter->setPostfields(['url' => SB_URL . '/apps/twitter/post.php'])->buildOauth('https://api.twitter.com/1.1/account_activity/all/sb/webhooks.json', 'POST')->performRequest(), true);
    if (sb_isset($response, 'valid') || (isset($response['errors']) && $response['errors'][0]['message'] == 'Too many resources already created.')) {
        $response = json_decode($twitter->buildOauth('https://api.twitter.com/1.1/account_activity/all/sb/subscriptions.json', 'POST')->performRequest(), true);
