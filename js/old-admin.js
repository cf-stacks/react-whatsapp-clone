                            ...
                            SBApps.dialogflow.translate(strings, SB_ADMIN_SETTINGS['active-agent-language'], (response) => {
                                if (response) {
                                    for (var i = 0; i < response.length; i++) {
                                        ...
                                        area.find(`[data-id="${message_ids[i]}"] .sb-message`).html((new SBMessage()).render(response[i].translatedText));
    ...
    $(document).ready(function () {
        ...
        */
        ...
        $(users_area).on('click', '.sb-new-user', function () {
            ...
            profile_edit_box.find('input,select,textara').removeClass('sb-error');
            if (SB_ACTIVE_AGENT['user_type'] == 'admin') {
                profile_edit_box.find('#user_type').find('select').html(`<option value="user">${sb_('User')}</option><option value="lead">${sb_('Lead')}</option><option value="agent">${sb_('Agent')}</option><option value="admin">${sb_('Admin')}</option>`);
            }
